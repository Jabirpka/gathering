...
function NotifIcon({ type }: { type: AppNotification['type'] }) {
  if (type === 'poke') return <Zap size={14} className="text-amber-400" />;
  if (type === 'approved') return <CheckCircle size={14} className="text-emerald-400" />;
  return <Bell size={14} className="text-brand-light" />;
}
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
            ...
            <div className="flex items-center gap-1">
              {notifications.length > 0 && (
                <button onClick={clear} className="text-xs text-slate-500 hover:text-slate-300 px-2 py-1 rounded">
                  Clear all
                </button>
              )}
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <div
                  ...
                  className={`flex items-start gap-3 px-4 py-3 border-b border-white/5 last:border-0 transition-colors ${n.groupId ? 'cursor-pointer hover:bg-white/5' : ''} ${!n.read ? 'bg-brand/5' : ''}`}
                >
